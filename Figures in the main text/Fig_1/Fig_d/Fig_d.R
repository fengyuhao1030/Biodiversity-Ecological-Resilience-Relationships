...
selectID <- which(tempMatrix$State == max(tempMatrix$State))
tippingpoint <- tempMatrix[selectID,]
# Arrow
arrow1 <- GenArrow(c(stableStateMatrix$Env[20],stableStateMatrix$State[20]),295,0.044)
arrow2 <- GenArrow(c(as.numeric(tippingpoint$Env[1]),0.6),0,0.044)
arrow3 <- GenArrow(c(stableStateMatrix$Env[310],stableStateMatrix$State[310]),272,0.044)

##==== Draw ====##
Fig <- ggplot()+
  geom_line(data = stableStateMatrix,mapping = aes(x = Env,y = State,group = factor(Type),color = Distance),linewidth = 1.7)+
  geom_line(data = stableStateMatrix,mapping = aes(x = Env,y = State),color = '#428a33',linewidth = 0.7)+
  geom_point(data = tippingpoint,mapping = aes(x = Env,y = State),shape = 21,size = 2,fill = '#428a33',color = '#000000',stroke = 0.4)+
  geom_polygon(data = arrow1,mapping = aes(x = X,y = Y),fill = '#428a33',color = '#000000',linewidth = 0.25)+
  geom_polygon(data = arrow2,mapping = aes(x = X,y = Y),fill = '#428a33',color = '#000000',linewidth = 0.25)+
...
write.csv(stableStateMatrix,file = 'Recovery.csv',row.names = FALSE)
##==== Draw ====##