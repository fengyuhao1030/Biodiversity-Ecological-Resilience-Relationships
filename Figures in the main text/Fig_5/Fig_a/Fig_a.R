...
for(i in seq(1,length(ecoVars))){
  fileName <- paste0('./Data/',ecoVars[i],'_TEM/High_Stable_State.csv')
...
Fig <- ggplot()+
  ...
  coord_flip()+
  theme_custom()
outputFileName <- 'High_Stable_TEM.pdf'
...
disCoeffMatrix <- data.frame()
for(i in seq(1,length(ecoVars))){
  fileName <- paste0('./Data/',ecoVars[i],'_PR/High_Stable_State.csv')
  dataMatrix <- read.csv(fileName)
  dataMatrix <- dataMatrix[seq(1,nrow(dataMatrix)-1),]
  selectIDs_1 <- which(dataMatrix$DiversityCoeff > 0)
  selectIDs_2 <- which(dataMatrix$DiversityCoeff < 0)
...
Fig <- ggplot()+
  ...
  geom_jitter(data = disCoeffMatrix,mapping = aes(x = X - offset*2,y = Coeff,group = factor(Class),color = factor(Color)),size = 0.03,width = 0.1,alpha = 0.4)+
  scale_x_continuous(limits = c(0.5,9.7),breaks = seq(1,9),labels = ecoVars[seq(9,1,-1)],expand = c(0,0))+
  scale_y_continuous(limits = c(-0.3,0.3),breaks = c(-0.2,0,0.2),labels = c('-0.2','0','0.2'),expand = c(0,0))+
  scale_fill_manual(values = c('#5469af','#bd494b'))+
  scale_color_manual(values = c('#5469af','#bd494b'))+
  coord_flip()+
  theme_custom()+
...
outputFileName <- 'High_Stable_PR.pdf'
...
disCoeffMatrix <- data.frame()
for(i in seq(1,length(ecoVars))){
  fileName <- paste0('./Data/',ecoVars[i],'_VPD/High_Stable_State.csv')
  dataMatrix <- read.csv(fileName)
  dataMatrix <- dataMatrix[seq(1,nrow(dataMatrix)-1),]
  selectIDs_1 <- which(dataMatrix$DiversityCoeff > 0)
  selectIDs_2 <- which(dataMatrix$DiversityCoeff < 0)
...
Fig <- ggplot()+
  ...
  theme(axis.line.y.left = element_blank(),
        axis.ticks.y.left = element_blank(),
        axis.text.y = element_blank())
outputFileName <- 'High_Stable_VPD.pdf'
pdf(file <- outputFileName,width = 1.07,height = 2.8)
print(Fig)
dev.off()
##==== VPD ====##
...
disCoeffMatrix <- data.frame()
for(i in seq(1,length(ecoVars))){
  fileName <- paste0('./Data/',ecoVars[i],'_AI/High_Stable_State.csv')
  dataMatrix <- read.csv(fileName)
  dataMatrix <- dataMatrix[seq(1,nrow(dataMatrix)-1),]
  selectIDs_1 <- which(dataMatrix$DiversityCoeff > 0)
  selectIDs_2 <- which(dataMatrix$DiversityCoeff < 0)
...
Fig <- ggplot()+
  ...
  theme(axis.line.y.left = element_blank(),
        axis.ticks.y.left = element_blank(),
        axis.text.y = element_blank())
outputFileName <- 'High_Stable_AI.pdf'
pdf(file <- outputFileName,width = 1.07,height = 2.8)
print(Fig)
dev.off()
##==== AI ====##
...
disCoeffMatrix <- data.frame()
for(i in seq(1,length(ecoVars))){
  fileName <- paste0('./Data/',ecoVars[i],'_PDSI/High_Stable_State.csv')
  dataMatrix <- read.csv(fileName)
  dataMatrix <- dataMatrix[seq(1,nrow(dataMatrix)-1),]
  selectIDs_1 <- which(dataMatrix$DiversityCoeff > 0)
  selectIDs_2 <- which(dataMatrix$DiversityCoeff < 0)
...
Fig <- ggplot()+
  ...
  theme(axis.line.y.left = element_blank(),
        axis.ticks.y.left = element_blank(),
        axis.text.y = element_blank())
outputFileName <- 'High_Stable_PDSI.pdf'
pdf(file <- outputFileName,width = 1.07,height = 2.8)
print(Fig)
dev.off()
##==== PDSI ====##