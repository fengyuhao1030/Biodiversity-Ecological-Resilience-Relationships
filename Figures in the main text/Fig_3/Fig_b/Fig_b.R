...
Fig <- ggplot()+
  geom_line(data = drawMatrix1,mapping = aes(x = X,y = Y),color = '#6bb52e',linewidth = 0.5)+
  scale_x_continuous(limits = c(-0.05,1.05),expand = c(0,0))+
  scale_y_continuous(limits = c(-0.7,0.2),expand = c(0,0))+
  theme_custom()
...
set.seed(100)
errors <- rnorm(n = 40,mean = 0,sd = 0.05)
tempX <- seq(0,1,length.out = 40)
tempY <- tempX*(-0.3) + errors
drawMatrix2 <- data.frame(X = tempX,Y = tempY)
Fig <- ggplot()+
  geom_line(data = drawMatrix2,mapping = aes(x = X,y = Y),color = '#961717',linewidth = 0.5)+
  scale_x_continuous(limits = c(-0.1,1.1),expand = c(0,0))+
  scale_y_continuous(limits = c(-0.7,0.3),expand = c(0,0))+
  theme_custom()
