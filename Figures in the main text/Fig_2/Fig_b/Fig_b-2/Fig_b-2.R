...
# Line
lineMatrix1 <- data.frame(X = c(as.numeric(tippingpoint1$Env),as.numeric(tippingpoint1$Env)),Y = c(as.numeric(tippingpoint1$State),as.numeric(tippingpoint1$State)-0.3))
lineMatrix2 <- data.frame(X = c(as.numeric(tippingpoint2$Env),as.numeric(tippingpoint2$Env)),Y = c(as.numeric(tippingpoint2$State),as.numeric(tippingpoint2$State)+0.3))
lineMatrix3 <- data.frame(X = c(as.numeric(tippingpoint3$Env),as.numeric(tippingpoint3$Env)),Y = c(as.numeric(tippingpoint3$State),as.numeric(tippingpoint3$State)-0.3))
lineMatrix4 <- data.frame(X = c(as.numeric(tippingpoint4$Env),as.numeric(tippingpoint4$Env)),Y = c(as.numeric(tippingpoint4$State),as.numeric(tippingpoint4$State)+0.3))
lineMatrix5 <- data.frame(X = c(as.numeric(tippingpoint5$Env),as.numeric(tippingpoint5$Env)),Y = c(as.numeric(tippingpoint5$State),as.numeric(tippingpoint5$State)-0.3))
lineMatrix6 <- data.frame(X = c(as.numeric(tippingpoint6$Env),as.numeric(tippingpoint6$Env)),Y = c(as.numeric(tippingpoint6$State),as.numeric(tippingpoint6$State)+0.3))
# Arrow
arrow1 <- GenArrow(c(dataMatrix1$Env[40],dataMatrix1$State[40]),110,0.065)
arrow2 <- GenArrow(c(lineMatrix1$X[2],lineMatrix1$Y[2]),180,0.065)

arrow3 <- GenArrow(c(dataMatrix2$Env[100],dataMatrix2$State[100]),272,0.065)
arrow4 <- GenArrow(c(lineMatrix2$X[2],lineMatrix2$Y[2]),0,0.065)

arrow5 <- GenArrow(c(dataMatrix3$Env[45],dataMatrix3$State[45]),130,0.065)
arrow6 <- GenArrow(c(lineMatrix3$X[2],lineMatrix3$Y[2]),180,0.065)

arrow7 <- GenArrow(c(dataMatrix4$Env[190],dataMatrix4$State[190]),272,0.065)
arrow8 <- GenArrow(c(lineMatrix4$X[2],lineMatrix4$Y[2]),0,0.065)

arrow9 <- GenArrow(c(dataMatrix5$Env[45],dataMatrix5$State[45]),140,0.065)
arrow10 <- GenArrow(c(lineMatrix5$X[2],lineMatrix5$Y[2]),180,0.065)

arrow11 <- GenArrow(c(dataMatrix6$Env[300],dataMatrix6$State[300]),272,0.065)
arrow12 <- GenArrow(c(lineMatrix6$X[2],lineMatrix6$Y[2]),0,0.065)
...
Fig <- ggplot()+
  ...
  scale_y_continuous(limits = c(-0.15,1.8),breaks = c(0,0.5,1,1.5),labels = c('0','0.5','1.0','1.5'),expand = c(0,0))+
  scale_color_gradient(low = '#e61c17',high = '#fbf300')+
  theme_custom()
outputFileName <- 'Fig_b-2.pdf'
