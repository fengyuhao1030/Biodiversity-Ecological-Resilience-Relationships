...
dataMatrix <- read.csv('./GIMMSKNDVI_TEM/High_Stable_State.csv')
dataMatrix <- dataMatrix[seq(1,nrow(dataMatrix)-1),]

dataMatrix$X <- 0
dataMatrix$Y <- 0
dataMatrix$Color <- 0
breaks <- seq(-0.04,0.28,0.006)
...
Fig <- ggplot()+
  ...
  geom_point(data = dataMatrix,mapping = aes(x = X,y = Y,color = factor(Color)),size = 0.2)+
  scale_x_continuous(limits = c(-0.05,0.29),breaks = c(0,0.1,0.2),labels = c('0','0.1','0.2'),expand = c(0,0))+
  scale_y_continuous(limits = c(-1.6,40),breaks = seq(0,40,20),expand = c(0,0))+
  scale_color_manual(values = c('#bd494b','#5469af'))+
  theme_custom()
